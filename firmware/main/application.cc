...
void Application::ActivationTask() {
    ...
    // Open the audio channel at boot so the device is reachable from the
    // gateway before any user-driven voice session starts. This is what
    // makes server-driven TTS push (e.g. an external producer sending
    // {"type":"tts","state":"start"} + binary Opus frames, such as the
    // SAIVerse persona_speak hook) work without requiring a physical
    // button press first. OpenAudioChannel() returns false if the gateway
    // is not yet reachable at boot; that is non-fatal — the reconnect
    // timer (websocket_protocol.cc::OpenAudioChannelInternal calls
    // ScheduleReconnect() on failure after upstream #197) will retry.
    //
    // Upstream PR #169 also keeps the WebSocket transport alive
    // independently of audio sessions at the protocol layer, so the two
    // layers (transport / audio channel) cooperate without further fork
    // logic.
    if (protocol_ != nullptr) {
        ESP_LOGI(TAG, "Opening audio channel at boot for server-driven push");
