...
class Gateway:
    ...
    async def start(self) -> None:
        ...
        # Start HTTP capture server (hosts /capture, /pcm, /avatar_set/{id}).
        # The PCM endpoint forwards into send_pcm_stream, so we hand the
        # active Gateway instance to the app for esp32 + tts_lock access.
        # The avatar set endpoint reuses the same web.Application; we keep
        # a reference to it so load_avatar_set can stage payloads against it.
